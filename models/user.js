...
const userSchema = new mongoose.Schema({
  ...
  _wall: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Wall' }],
  _emotions: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Wall' }]
},
  {
    timestamps: true,
  });
...
userSchema.statics.edit = (userId, userObj, cb) => {
  User.findOne({ _id: userId }, (err, dbUser) => {
    bcrypt.hash(userObj.password, 12, (err, hash) => {
      ...
      dbUser.phone = userObj.phone;
      dbUser.address = userObj.address;
